...
//   • category cutoffs low < 4, medium 4–<7, high >= 7
//   • output shape matches POST /predict (addiction_level, category,
...
export function fieldContribution(key, value) {
  const f = FIELDS[key];
  if (!f) return 0;
...
export function computeResult(values) {
...
//  unreachable / the free Space is cold-starting, it falls back to the local
//  demo model so the page never breaks. To force local-only, just
//  `return computeResult(values)`.
// ════════════════════════════════════════════════════════════════════════

// API recommendations are plain strings; the UI cards want { text, icon }.
const REC_ICON_RULES = [
  [/tidur|sebelum tidur/i, "moon"],
  [/medsos|sosial|interaksi/i, "users"],
  [/keluarga|ngobrol/i, "message"],
  [/olahraga|fisik|game|main/i, "activity"],
  [/napas|cemas|perasaan|syukur|percaya/i, "heart"],
  [/notifikasi|app|aplikasi|layar|hp|batas|produktif|eduka/i, "smartphone"],
];
function recIcon(text) {
  for (const [re, icon] of REC_ICON_RULES) if (re.test(text)) return icon;
  return "sparkles";
}

// Reshape the backend payload into the exact shape the UI components consume.
function normalizeResult(api) {
  return {
    addiction_level: api.addiction_level,
    category: api.category,
    category_label: api.category_label,
    interpretation: api.interpretation,
    recommendations: (api.recommendations || []).map((r) =>
      typeof r === "string" ? { text: r, icon: recIcon(r) } : r,
    ),
    // DriverChart needs a stable React key; the API driver's `feature` is unique.
    drivers: (api.drivers || []).map((d) => ({ ...d, key: d.key ?? d.feature })),
  };
}
