...
function Background({ calm = 0 }) {
  ...
  const targetRef = bgUseRef(calm);
  const stateRef = bgUseRef({ calm, particles: [], raf: 0, w: 0, h: 0, dpr: 1 });
  const reduced = bgUseRef(false);

  bgUseEffect(() => { targetRef.current = calm; }, [calm]);
  ...
  bgUseEffect(() => {
    ...
    function spawn() {
      return { x: rand(0, S.w), y: rand(0, S.h), r: rand(0.6, 1.8),
    ...
    function frame(now) {
      ...
      for (let i = 0; i < S.particles.length; i++) {
        ...
        if (p.x < -5) p.x = S.w + 5; else if (p.x > S.w + 5) p.x = -5;
        if (p.y < -5) p.y = S.h + 5; else if (p.y > S.h + 5) p.y = -5;
  ...
  return React.createElement("div", { className: "fixed inset-0 -z-10 overflow-hidden", "aria-hidden": "true" },
